...
利用者：案件の「見積依頼」タブを開く
  ↓
AI入札部：「営業AIで新しい取引先を探して打診する」を出す
      （依頼先がすでにいる業種でも出す。ユーザー決定 2026-08-29。
       対応表に無い業種では出さない）
  ↓
利用者：「何社いるか見る」
  ↓
...
`createTenant()`（`packages/outreach/adapters/eigyou_ai.ts`）も**必ず渡すようにした**（2026-08-29）。

**「既定値のままで足りている」は誤りだった。**
既定値は `config.FORM_MAX_PER_TENANT_PER_MONTH_DEFAULT = 4000`（通/月）で、
...
気づかないまま契約より多く配るより、作成に失敗して直すほうがよい。

`/admin/sales-ai` のフォームに入力欄は置いていない（プランが1つしか無いため）。
プランを増やすときに `basePlanQuota()` を分岐させる。
